...
def find_optimal_k_kmeans(X, k_range=None):
    if k_range is None:
        k_range = range(2, 11)
    """
    ...
    return {"k_values": list(k_range), "inertias": inertias, "silhouette_scores": silhouette_scores, "best_k": best_k}
...
def visualize_clusters(X, results, true_labels=None):
    ...
    for name, result in results.items():
        ...
        if name == "DBSCAN" and -1 in labels:
            noise_mask = labels == -1
            ax.scatter(X_2d[noise_mask, 0], X_2d[noise_mask, 1], c="red", marker="x", s=100, label="Noise", alpha=0.8)
            ax.legend()
...
def complete_clustering_analysis(X, true_labels=None, scale=True, find_k=True, k_range=None, n_clusters=3):
    if k_range is None:
        k_range = range(2, 11)
    """
    ...
    return {"X_processed": X_processed, "optimization": optimization_results, "comparison": comparison_results}
...
if __name__ == "__main__":
    ...
    X_synth, y_synth = make_blobs(n_samples=500, n_features=2, centers=4, cluster_std=0.5, random_state=42)
    ...
    results_synth = complete_clustering_analysis(
        X_synth, true_labels=y_synth_with_noise, scale=True, find_k=True, k_range=range(2, 8), n_clusters=4
    )
