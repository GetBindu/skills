...
def generate_qc_plots(adata, output_prefix="qc"):
    ...
    # Scatter plots
    sc.pl.scatter(adata, x="total_counts", y="pct_counts_mt", save=f"_{output_prefix}_mt_scatter.pdf")
    sc.pl.scatter(adata, x="total_counts", y="n_genes_by_counts", save=f"_{output_prefix}_genes_scatter.pdf")
...
def filter_data(adata, mt_threshold=5, min_genes=200, max_genes=None, min_counts=None, max_counts=None, min_cells=3):
    """
...
def main():
    ...
    parser.add_argument("--output", default="qc_filtered.h5ad", help="Output file name (default: qc_filtered.h5ad)")
    parser.add_argument("--mt-threshold", type=float, default=5, help="Max mitochondrial percentage (default: 5)")
    ...
    adata = calculate_qc_metrics(
        adata, mt_threshold=args.mt_threshold, min_genes=args.min_genes, min_cells=args.min_cells
    )
    ...
    # Filter data
    adata = filter_data(adata, mt_threshold=args.mt_threshold, min_genes=args.min_genes, min_cells=args.min_cells)
