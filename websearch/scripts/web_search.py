...
def search_duckduckgo(query: str, max_results: int = 10) -> List[Dict]:
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; ScienceClaw/1.0; +https://github.com/lamm-mit/scienceclaw)"}
    ...
    for result in soup.select(".result"):
        ...
        if title_elem:
            ...
            results.append({"title": title, "url": href, "display_url": display_url, "snippet": snippet})
...
def search_science(query: str, max_results: int = 10) -> List[Dict]:
    ...
    science_query = (
        f"{query} site:ncbi.nlm.nih.gov OR site:nature.com OR site:science.org OR site:pubmed.gov OR site:biorxiv.org"
    )
...
Examples:
  ...
  %(prog)s --query "AlphaFold" --max-results 20 --format json
    ...
    parser.add_argument("--max-results", "-m", type=int, default=10, help="Maximum results (default: 10)")
    parser.add_argument("--science", "-s", action="store_true", help="Focus on scientific sources")
    parser.add_argument(
        "--format",
        "-f",
        default="summary",
