...
def search_proteins(query: str, format: str = "json", fields: Optional[List[str]] = None, size: int = 25) -> Dict:
    """
...
def batch_retrieve(accessions: List[str], format: str = "json", fields: Optional[List[str]] = None) -> str:
    """
...
def stream_results(
    query: str, format: str = "fasta", fields: Optional[List[str]] = None, chunk_size: int = 8192
...
Examples:
  ...
  %(prog)s --list-databases
    ...
    group.add_argument("--list-databases", action="store_true", help="List all ID mapping databases")

    # Format options
    parser.add_argument("--format", "-f", default="json", help="Output format (json, tsv, xlsx, xml, fasta, txt, rdf)")

    # Search-specific options
    parser.add_argument("--fields", help="Comma-separated list of fields to return")
    parser.add_argument("--size", type=int, default=25, help="Number of results (default: 25, max: 500)")
    ...
    try:
        if args.list_fields:
            ...
            results = search_proteins(args.search, format=args.format, fields=fields_list, size=args.size)
            if args.format == "json":
